...
class Calendar {
  ...
  $tail = '';
  $ldopm = new \DateTime('last day of ' . $this->yearMonth .' -1 month');
  while ($ldopm->format('w') < 6) {
    $tail = sprintf('<td class="gray">%d</td>', $ldopm->format('d')).$tail;
    $ldopm->sub(new \DateInterval('P1D'));
  }
  return $tail;
}
...
private function _getHead() {
  $head = '';
  $nd = new \DateTime('first day of'. $this->yearMonth .'+1 month');
  while($nd->format('w') > 0) {
    $head .= sprintf('<td class="gray">%d</td>',$nd->format('d'));
